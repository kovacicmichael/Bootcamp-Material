...
app.post("/submit", function(req, res) {
  ...
  //book was already defined on the front end, in the req.body
...
app.get("/markread/:id", function(req, res) {
  ...
  db.books.update({_id: mongojs.ObjectId(req.params.id)}, {$set:{read: true}}, function(err, updated){
    if(err){
      conosle.log(err)
    }else{
      console.log(updated)
...
app.get("/markunread/:id", function(req, res) {
  // Remember: when searching by an id, the id needs to be passed in
  // as (mongojs.ObjectId(IDYOUWANTTOFIND))
  db.books.update({_id: mongojs.ObjectId(req.params.id)}, {$set:{read: false}}, function(err, updated){
    if(err){
      conosle.log(err)
    }else{
      console.log(updated)
